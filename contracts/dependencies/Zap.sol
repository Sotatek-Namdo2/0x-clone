...
contract Zap is OwnableUpgradeable {
    ...
    function zapInToken(ZapInForm calldata _params) public returns (uint256 liquidity) {
        ...
        liquidity = _addLiquidity(protocols[_params.protocolType].router, token0, token1, _params.receiver);
    ...
    function zapOut(
        ...
        IJoeRouter02(router).swapExactTokensForTokens(token1Balance, 0, tempPath, _receiver, block.timestamp);
    ...
    function _swapETHToLP(
        ...
        if (token0 == wrappedNative || token1 == wrappedNative) {
            ...
            IJoeRouter02(router).addLiquidityAVAX{ value: _amount - swapValue }(
                token,
                tokenAmount,
                0,
                0,
